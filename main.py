...
# The program expects Tesseract to be already installed and ready on the computer.
# ---------------------------------
...
def initialize_yolo_model(weights_path):
    ...
    try:
        if not os.path.exists(weights_path):
            # Fallback path if you run it locally and models are in a known location
            local_path = os.path.join(os.path.dirname(__file__), weights_path)
...
# --- 4. LICENSE PLATE DATABASE SETUP & LISTS ---
...
# --- STOLEN VEHICLE BLACKLIST ---
STOLEN_VEHICLE_BLACKLIST = [
    "LSI5EBC", # Example plate 1
    "KL07CD5",
    "ML05KF5", # Example plate 2
    "LS15EBC"  # Example plate 3
]
# -----------------------------------------------

# --- EMERGENCY VEHICLE WHITELIST (NEW ADDITION) ---
# Vehicles detected here will receive a priority alert (Green box).
EMERGENCY_VEHICLE_WHITELIST = [
    "R183JF",
    "FIR9012", # Example Ambulance
    "RIB3JF", # Example Police
    "FIR9012"  # Example Fire Truck
]
# ----------------------------------------------------
...
def tesseract_ocr_process(frame, x1, y1, x2, y2):
    ...
    # Add a small buffer to the crop area for better OCR results
    buffer = 5
    x1 = max(0, x1 - buffer)
    y1 = max(0, y1 - buffer)
    x2 = min(w, x2 + buffer)
    ...
    try:
        ...
        # Apply slight contrast adjustment (CLAHE) for better plate visibility 
    ...
    # Removes symbols, converts to uppercase, and removes spaces.
...
def save_to_lp_database(license_plates, start_time, end_time):
    ...
    # We only save normal and STOLEN plates to the main DB for tracking.
    # EMERGENCY alerts are logged, but treated as normal plates for DB storage.
    plates_to_log = {p.split(':')[-1] for p in license_plates if not p.startswith("OCR_") and not p.startswith("INVALID_")}

    for plate in plates_to_log:
        cursor.execute('''
...
def process_lp_frame(frame, license_plates_set, model):
    ...
    # Flags for real-time Streamlit alerts
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            # --- WHITELIST / BLACKLIST CHECK LOGIC (UPDATED) ---
            box_color = (255, 0, 0) # Default color: Blue (BGR)
            display_label = label if label and not label.startswith("OCR_") and not label.startswith("INVALID_") else f'{classNameInt}:{conf:.2f}'

            if label and label in EMERGENCY_VEHICLE_WHITELIST:
                # MATCH FOUND! Emergency Vehicle. Set color to GREEN.
                box_color = (0, 255, 0) # Green (BGR)
                emergency_vehicle_detected = True
                display_label = f"🟢 EMERGENCY: {label}"
                # Log the alert (placeholder string for set)
                license_plates_set.add(f"EMERGENCY_VEHICLE_DETECTED:{label}")
                
            elif label and label in STOLEN_VEHICLE_BLACKLIST:
                # MATCH FOUND! Stolen Vehicle. Set color to RED.
                box_color = (0, 0, 255) # Red (BGR)
                stolen_vehicle_detected = True
                display_label = f"🚨 STOLEN: {label}"
                # Log the alert (placeholder string for set)
                license_plates_set.add(f"STOLEN_VEHICLE_DETECTED:{label}")
            else:
                # Normal Vehicle (Blue)
                box_color = (255, 0, 0)
                # Add the unique plate number to the set of collected plates (for non-special logging)
                if label and not label.startswith("OCR_") and not label.startswith("INVALID_"):
                    license_plates_set.add(label)
            # --- END WHITELIST / BLACKLIST CHECK LOGIC ---
                
            # Draw a box around the detected license plate on the image.
    ...
    # Display real-time Streamlit alerts
    if stolen_vehicle_detected:
        st.error("🚨 STOLEN VEHICLE ALERT! A blacklisted license plate was detected in this frame.", icon="🚨")
    if emergency_vehicle_detected:
        st.warning("🟢 EMERGENCY VEHICLE DETECTED! Priority access required.", icon="🟢")
...
def lp_video_processing_loop(cap, model):
    ...
    while cap.isOpened():
        ...
        if (currentTime - startTime).seconds >= 20:
            endTime = currentTime
            # We filter out the special alert tags before saving to standard JSON/DB
            plates_to_save = {p for p in license_plates if not p.startswith("STOLEN_VEHICLE_DETECTED:") and not p.startswith("EMERGENCY_VEHICLE_DETECTED:")}
            save_lp_json(plates_to_save, startTime, endTime)
            startTime = currentTime
            license_plates.clear()

        # Prepare list for display, including stolen and emergency alerts
        display_list = []
        stolen_count = 0
        emergency_count = 0
        
        for item in license_plates:
            if item.startswith("STOLEN_VEHICLE_DETECTED:"):
                display_list.append(f"🚨 {item.replace('STOLEN_VEHICLE_DETECTED:', '')}")
                stolen_count += 1
            elif item.startswith("EMERGENCY_VEHICLE_DETECTED:"): # NEW Check
                display_list.append(f"🟢 {item.replace('EMERGENCY_VEHICLE_DETECTED:', '')}")
                emergency_count += 1
            else:
                display_list.append(item)
        
        # Update the status information displayed to the user.
        status_placeholder.text(f"Frames processed: {frame_count} | Unique Entries: {len(display_list)} (since last save) | STOLEN ALERTS: {stolen_count} | EMERGENCY ALERTS: {emergency_count}")
        plate_placeholder.json({"Detected Entries (since last save)": list(display_list)})
    ...
    if license_plates:
        plates_to_save = {p for p in license_plates if not p.startswith("STOLEN_VEHICLE_DETECTED:") and not p.startswith("EMERGENCY_VEHICLE_DETECTED:")}
        save_lp_json(plates_to_save, startTime, datetime.now())
...
# --- PIE CHART FUNCTION ---
def create_pie_chart(class_counts_dict):
    """Generates a Matplotlib Pie Chart from vehicle class counts."""
    if not class_counts_dict:
        st.info("No vehicle data to generate a pie chart.")
        return None

    labels = list(class_counts_dict.keys())
    sizes = list(class_counts_dict.values())
    
    total = sum(sizes)
    if total == 0:
          st.info("Total vehicle count is zero. Cannot create chart.")
          return None
          
    def autopct_format(values):
        def my_format(pct):
            sizes_percent = [s / total * 100 for s in sizes]
            closest_size_index = min(range(len(sizes_percent)), key=lambda i: abs(sizes_percent[i] - pct))
            count = sizes[closest_size_index]
            
            return f'{pct:.1f}%\n({count})'
        return my_format
        
    fig, ax = plt.subplots(figsize=(8, 8))
    max_count_index = sizes.index(max(sizes))
    explode = [0.05 if i == max_count_index else 0 for i in range(len(sizes))] 

    ax.pie(
        sizes, 
        labels=labels, 
        autopct=autopct_format([s / total * 100 for s in sizes]), 
        startangle=90, 
...
def apply_custom_styles():
    ...
    st.markdown("""
    <style>
    /* ... (CSS styles for traffic level display remain here) ... */
    </style>
...
def license_plate_mode(model):
    ...
    # NEW: Stolen Vehicle Blacklist Display
    st.sidebar.subheader("🚨 Stolen Vehicle Blacklist")
    st.sidebar.caption(f"Checking against **{len(STOLEN_VEHICLE_BLACKLIST)}** plates.")
    st.sidebar.markdown(f"`{', '.join(STOLEN_VEHICLE_BLACKLIST)}`")
    st.sidebar.markdown("---")

    # NEW: Emergency Vehicle Whitelist Display
    st.sidebar.subheader("🟢 Emergency Vehicle Whitelist")
    st.sidebar.caption(f"Checking against **{len(EMERGENCY_VEHICLE_WHITELIST)}** plates.")
    st.sidebar.markdown(f"`{', '.join(EMERGENCY_VEHICLE_WHITELIST)}`")
    st.sidebar.markdown("---")
        ...
        if uploaded_image is not None:
            ...
            with col2:
                if st.button("Analyze Photo 🖼️"):
                    ...
                    if license_plates:
                        # Filter out the special alert tags before saving
                        plates_to_save = {p for p in license_plates if not p.startswith("STOLEN_VEHICLE_DETECTED:") and not p.startswith("EMERGENCY_VEHICLE_DETECTED:")}
                        
                        stolen_plates = {p.replace("STOLEN_VEHICLE_DETECTED:", "") for p in license_plates if p.startswith("STOLEN_VEHICLE_DETECTED:")}
                        emergency_plates = {p.replace("EMERGENCY_VEHICLE_DETECTED:", "") for p in license_plates if p.startswith("EMERGENCY_VEHICLE_DETECTED:")}
                        
                        if stolen_plates:
                            st.error(f"🚨 ALERT! STOLEN PLATES DETECTED: {', '.join(stolen_plates)}")
                        
                        if emergency_plates:
                            st.warning(f"🟢 ALERT! EMERGENCY PLATES DETECTED: {', '.join(emergency_plates)}")
                        
                        if plates_to_save:
                            st.success(f"Analysis Complete! Detected {len(plates_to_save)} unique normal entries.")
                            st.json(list(plates_to_save))

                        current_time = datetime.now()
                        # Save the results immediately since it's a single image.
                        save_lp_json(plates_to_save.union(stolen_plates).union(emergency_plates), current_time, current_time) # Ensure all unique plates are saved
                    else:
...
def atcc_mode(model, db: TrafficDB):
    ...
    if results_summary:
        ...
        st.subheader("Vehicle Class Distribution Pie Chart 📊")
        if class_counts_dict:
            pie_chart_buffer = create_pie_chart(class_counts_dict)
            if pie_chart_buffer:
                st.image(pie_chart_buffer, use_container_width=True)
                st.success(f"Showing distribution of {results_summary['total_vehicles']} vehicles.")
        else:
            st.info("No vehicle data from the current analysis to create a pie chart.")
